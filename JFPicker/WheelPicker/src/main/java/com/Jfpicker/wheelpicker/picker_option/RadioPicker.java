...
import com.Jfpicker.wheelpicker.picker_option.adapter.RadioAdapter;
...
/**
 * @author Created by JF on  2022/6/5 15:58
 * @description 单选弹窗，默认居中，列表项包含一个id是tvRecyclerviewContent的TextView 一个id是ivCheck的ImageView
 * 可以设置布局id，选中未选中图片id实现自定义样式
 * 也可以通过设置OnRecyclerviewStyleListener实现自定义样式
 */

public class RadioPicker extends ModalDialog {
    ...
    public RadioPicker(@NonNull Activity activity) {
        super(activity, DialogConfig.getCenterConfig(10));
    }

    public RadioPicker(@NonNull Activity activity, DialogConfig dialogConfig) {
        super(activity, (dialogConfig == null) ? DialogConfig.getCenterConfig(10) : dialogConfig);
    }

    public RadioPicker(@NonNull Activity activity, int themeResId) {
        super(activity, DialogConfig.getCenterConfig(10), themeResId);
    }

    public RadioPicker(@NonNull Activity activity, DialogConfig dialogConfig, @StyleRes int themeResId) {
        ...
        if (onOptionPickedListener != null) {
            for (int i = 0; i < mAdapter.getDataList().size(); i++) {
                if (mAdapter.getDataList().get(i).isChecked()) {
                    onOptionPickedListener.onOption(mAdapter.getDataList().get(i).getId(),
                            mAdapter.getDataList().get(i).getName());
                    break;
                }
            }
        }
    ...
    public void setOptionsData(List<OptionEntity> list) {
        ...
        mAdapter = new RadioAdapter(getContext(), list);
    ...
    public void setOptionsData(List<OptionEntity> list,
        ...
        recyclerView.setLayoutManager(new LinearLayoutManager(getContext()));
        mAdapter = new RadioAdapter(getContext(), list, onRecyclerviewStyleListener);
        recyclerView.setAdapter(mAdapter);
    }

    public void setOptionsData(int layoutResId, List<OptionEntity> list) {
        recyclerView.setLayoutManager(new LinearLayoutManager(getContext()));
        mAdapter = new RadioAdapter(getContext(), layoutResId, list);
        recyclerView.setAdapter(mAdapter);
    }

    public void setOptionsData(int uncheckedIcon, int checkedIcon, List<OptionEntity> list) {
        recyclerView.setLayoutManager(new LinearLayoutManager(getContext()));
        mAdapter = new RadioAdapter(getContext(), uncheckedIcon, checkedIcon, list);
        recyclerView.setAdapter(mAdapter);
    }

    public void setOptionsData(int layoutResId, int uncheckedIcon, int checkedIcon, List<OptionEntity> list) {
        recyclerView.setLayoutManager(new LinearLayoutManager(getContext()));
        mAdapter = new RadioAdapter(getContext(), layoutResId, uncheckedIcon, checkedIcon, list);
        recyclerView.setAdapter(mAdapter);
    }
